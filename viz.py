...
def submit():
    ...
    if len(request.form['query']) > 200:
        return render_template("viz.html", displaydata={}, errordata=4, campus=campus)
    elif re.compile(r'[^ a-zA-Z]').search(request.form['query']):
        return render_template("viz.html", displaydata={}, errordata=2, campus=campus)
    else:
    ...
    resp = requests.get('http://onesearch.cuny.edu/PrimoWebServices/xservice/search/brief?&institution={}&query=any,contains,{}&query=facet_rtype,exact,books&indx=1&loc=local,scope:(KB,AL,CUNY_BEPRESS)&loc=adaptor,primo_central_multiple_fe&json=true'.format(campus_code, query))
    ...
    if readydata == False:
        return render_template("viz.html", displaydata={}, errordata=1, campus=campus) 
    else:
